...
defineProps<{ onSearch: () => Promise<void> | void }>();
...
<template>
  <div class="flex flex-col lg:flex-row lg:items-center justify-start gap-4 px-5 py-3 lg:py-2.5 border-b border-default bg-default shrink-0">
    <div class="flex flex-col sm:flex-row sm:items-center gap-2 shrink-0">
      <span class="text-[10px] font-semibold uppercase tracking-widest text-muted">Dispositivo</span>
      <USelect v-model="selectedDevice" :items="listDevice" class="w-full lg:w-44" size="sm" />
    </div>

    <div class="hidden lg:block w-px h-5 bg-default shrink-0" />
...
<UButton
  ...
  loading-auto
  class="w-full lg:w-auto justify-center shrink-0 lg:ml-2"
  @click="onSearch"
/>
  </div>
</template>
...
.dp__input { height: 30px !important; font-size: 0.75rem !important; line-height: 1.25rem !important; }

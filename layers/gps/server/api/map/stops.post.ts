import { getStopData } from '../../services/Map/Map.service'
...
const getMapStopsCached = defineCachedFunction(
  async (devices: string[], startISO: string, endISO: string) => {
    ...
    return await getStopData(client, devices, startISO, endISO)
  },
  {
    name: 'gps:map:stops',
    maxAge: cacheMaxAge.mapStops,
    swr: true,
...
export default defineEventHandler(async (event) => {
  ...
  try {
    return await getMapStopsCached(devices, start.toISOString(), end.toISOString())
  } catch (error) {
    if ((error as any)?.statusCode) throw error
    throw createError({ statusCode: 500, message: 'Error en reporte de inactividad.' })
  }
})

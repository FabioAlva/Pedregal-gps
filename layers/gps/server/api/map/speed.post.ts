...
const getMapSpeedCached = defineCachedFunction(
  async (devices: string[], startISO: string, endISO: string) => {
    ...
    return await getSpeedData(client, devices, startISO, endISO)
  },
  {
    name: 'gps:map:speed',
    maxAge: cacheMaxAge.mapSpeed,
    swr: true,
...
export default defineEventHandler(async (event) => {
  ...
  try {
    return await getMapSpeedCached(devices, start.toISOString(), end.toISOString())
  } catch (error) {
    if ((error as any)?.statusCode) throw error
    throw createError({ statusCode: 500, message: 'Error en reporte de velocidades.' })
  }
})

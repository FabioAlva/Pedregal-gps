import { getRouteAnalysisData } from '../../services/Map/Map.service'
...
const getMapRouteAnalysisCached = defineCachedFunction(
  async (devices: string[], startISO: string, endISO: string) => {
    ...
    return await getRouteAnalysisData(client, devices, startISO, endISO)
  },
  {
    name: 'gps:map:route-analysis',
    maxAge: cacheMaxAge.mapRouteAnalysis,
    swr: true,
...
export default defineEventHandler(async (event) => {
  ...
  try {
    return await getMapRouteAnalysisCached(devices, start.toISOString(), end.toISOString())
  } catch (error) {
    if ((error as any)?.statusCode) throw error
    throw createError({ statusCode: 500, message: 'Error en analisis de trayectos.' })
  }
})

import { getDistanceStats } from '../../services/Map/Map.service'
...
const getMapDistanceCached = defineCachedFunction(
  async (devices: string[], startISO: string, endISO: string) => {
    ...
    return await getDistanceStats(client, devices, startISO, endISO)
  },
  {
    name: 'gps:map:distance',
    maxAge: cacheMaxAge.mapDistance,
    swr: true,
...
export default defineEventHandler(async (event) => {
  ...
  try {
    return await getMapDistanceCached(devices, start.toISOString(), end.toISOString())
  } catch (error) {
    if ((error as any)?.statusCode) throw error
    throw createError({ statusCode: 500, message: 'Error en reporte de kilometraje.' })
  }
})
